...
You are Gemini CLI, working on a Python project with strict Test-Driven Development (TDD) and code quality standards.
...
### Using Gemini CLI Tools

```bash
# Read files
cat src/python_modern_template/module.py
...
**Manual workflows available (see `AI_DOCS/ai-skills.md` for detailed instructions):**
- `test-generator` - Generate test boilerplate
- `coverage-analyzer` - Analyze coverage gaps
- `quality-fixer` - Apply automated fixes
- `session-template` - Load task templates
- `tdd-reviewer` - Review TDD compliance
- `quality-enforcer` - Enforce quality gates

**Note**: Gemini CLI doesn't have direct Skill/Task tool access like Claude Code. Follow manual workflows documented in `AI_DOCS/ai-skills.md`.
...
- `GEMINI.md` - Gemini CLI (this file)
